...
To set up the environment we will use to create the report we just need to 
...
HolidaysHomePageTable <-
  data.table(
  country = html_text(HolidaysHomePage) |>
    str_match("\\w.+\\w$")|>
    as.vector(),
  link = html_attr(HolidaysHomePage,"href")
  )[country %chin% unique(ProtestCleanTidy[["country"]])]


# I tried to use lappy but it was just return NULL values in the list

ListToFill <- vector(mode = "list")

for(i in seq_len(nrow(HolidaysHomePageTable))){
  
  ListToFill <-
    append(
      ListToFill,
      list(read_html(HolidaysHomePageTable[["link"]][i]) |>
             html_table(header = TRUE) |>
             (\(LIST) as.data.table(LIST[[1]]))() |>
             (\(DT) DT[, country := 
...
CountriesHolidaysTable <-
  rbindlist(ListToFill, fill = TRUE, use.names = TRUE
  )[, c("country","Date","Holiday Name", "Type","Comments")
  ][, id := .I
  ][, .SD[CJ(id = id,
             year = min(ProtestCleanTidy[["year"]]):max(ProtestCleanTidy[["year"]])),
          on = "id"]
...
StateReponseMatrix <-
  ProtestCleanTidy[action_source == "state_response"
  ][, action_occur := as.double(action_occur)
  ][, dcast(.SD, id ~ actions, value.var = "action_occur")
  ][, !c("id")]

ggcorrplot(cor(StateReponseMatrix),
           hc.order = TRUE,
           type = "upper",
           lab = TRUE,
           p.mat = cor_pmat(StateReponseMatrix)) +
  labs(title = "TITLE TEST")


DemandsMatrix <-
  ProtestCleanTidy[action_source == "protester_demand"
  ][, action_occur := as.double(action_occur)
  ][, dcast(.SD, id ~ actions, value.var = "action_occur")
  ][, !c("id")]

ggcorrplot(cor(DemandsMatrix),
           hc.order = TRUE,
           type = "upper",
           lab = TRUE,
           p.mat = cor_pmat(DemandsMatrix)) +
  labs(title = "TITLE TEST")
