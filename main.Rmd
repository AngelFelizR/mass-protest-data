...
h1.title{font-size: 30px; font-weight: bold; color: green; }
...
.list-group-item.active, .list-group-item.active:focus, .list-group-item.active:hover{background-color: green; }

#rmd-show-all-code:hover, #rmd-hide-all-code:hover {background-color: green; }
...
## 1.3. Defining custom functions

In this section we will define some custom functions that will make the come cleaner and easier to change and improve over time:

- **count_category**: It counts the unique number of *id_var* for category included in *group_var* returning a list with a plot and a vector with the levels of the *group_var*.
  ...
  - **plot_max**:  A numeric value that defines the number of categories that will appear in the plot.
...
protest_data_tidy[id == id[1], !c("protesteridentity","notes","sources")
                ][, id := as.character(id)] |>
  flextable() |>
  theme_vanilla() |>
  autofit()
...
# This vector will help me to use the same color for each continent
# in the whole project
europe_levels <-levels(europe_summary$country)


ggplot(europe_summary[1:15],
       aes(number_protest, fct_rev(country), fill = country))+
  geom_col()+
  geom_text(aes(label = percent(pct, accuracy = 0.1)), hjust = -0.3, size = 6)+
  geom_blank(aes(x = number_protest *1.08))+
  scale_x_continuous(labels = comma_format(accuracy = 1))+
  labs(title = "Number of Protests per Continent", x = "", y = "" )+
  theme(legend.position = "none",
        plot.title = element_text(color = "black", face = "bold", 
                                  size = 22, hjust = 0.5),
        axis.title = element_text(color = "black", face = "bold", size = 18),
