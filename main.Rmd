...
- Graph visuatilation with **igraph** and **ggraph**
...
## 1.1. Libraries and data importation
...
# They help to extract the number of protestants
...
## 2.1. Initial data Status

As we can see bellow most of the rows represent a protest.
...
ProtestRaw[, .(`Number of rows` = .N / max(.I)), 
  ...
  scale_fill_manual(values = c("No Protest" = "gray80", 
                               "Protest" = "seagreen"))+
  coord_polar(theta = "y")+
  labs(title = "Propotion of rows that represent a protest", fill = "") +
  theme_void()+
...
When a row doesn't represent a protest has lost of missing data
...
Dates are store in different columns

```{r}
ProtestRaw[1:5, .SD, 
           .SDcols = patterns("^id$|day$|month$|year$")
  ][, id := as.character(id)] |>
  style_table()
```
