...
Polity5 <-
  ...
  )[, goverment_status := fcase(polity == -66, "Interruption",
                                polity == -77, "Interregnum",
                                polity == -88, "Transition",
                                default = "Active")
  ][goverment_status != "Active", 
...
A we can see the data have some numeric and logical variable that can be converted to numeric. It would be really interesting if we can confirm which variables go down or go up together. To find out that we can use the `correlation` function from the **correlation** with also give us confident intervals to confirm that the correlation are statistician significance.

```{r}

CorSummary <-
  ProtestLocationExtracted[, !c("id","ccode","democ","autoc", "durable","year")
  ][, participants_category := as.integer(participants_category)
  ][, lapply(.SD, \(x) x * 1),
    .SDcols = \(x) is.logical(x) | is.numeric(x)
  ][, correlation::correlation(.SD) |> as.data.table()
  ][p < 0.05
  ][order(-r)
  ][1:40]

# "nicely"  "with_fr" "with_graphopt" "with_kk" "with_lgl" "with_graphopt"
set.seed(50) # 15
graph_from_data_frame(CorSummary, directed = FALSE) |>
  ggraph(layout = "with_kk")+
  geom_edge_link(aes(edge_alpha = r), edge_width = 1,
                 edge_color = "blue")+
  geom_node_point(size = 3)+
  geom_node_text(aes(label = name), repel = TRUE)+
  labs(title = "What are the biggest correlations?")+
  theme(legend.position = "top",
        legend.title = element_blank())

```

## How does political factor affects state reponse?
...
## Does the number of participants increase any state repose?

```{r}

melt.data.table(ProtestLocationExtracted[!is.na(participants_category)],
                id.vars = c("id","participants_category"),
                measure.vars = grep("^response",names(ProtestLocationExtracted),
                                    value = TRUE),
                value.name = "The event happen") |>
  ggplot(aes(participants_category, fill = `The event happen`))+
  geom_bar(position = position_fill(), alpha = 0.85)+
...
melt.data.table(ProtestLocationExtracted,
                id.vars = c("id","response_accommodation"),
                measure.vars = c("polity","autoc","democ"),
                value.name = "index_value") |>
  ggplot(aes(response_accommodation, index_value))+
  geom_boxplot(aes(fill = response_accommodation))+
  facet_wrap(~variable)+
...
ggplot(ProtestLocationExtracted, aes(num_cities))+
  geom_histogram(aes(fill = response_accommodation), binwidth = 2)+
  facet_wrap(~response_accommodation, ncol = 1, scales = "free_y")
...
ggplot(ProtestLocationExtracted, aes(total_population))+
  geom_histogram(aes(fill = response_accommodation))+
  scale_x_log10()+
  facet_wrap(~response_accommodation, ncol = 1, scales = "free_y")
...
ProtestLocationExtracted[, .(id,
                             response_accommodation,
                             duration = difftime(end_date,start_date, units = "days") |>
                               as.double())] |>
  ggplot(aes(response_accommodation, duration))+
  geom_jitter(aes(color = response_accommodation), alpha = 0.3)+
