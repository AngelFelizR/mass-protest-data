...
#          ][, .(Country,
#                `Capital City`,
#                `Capital Population` = clean_amount(Population),
#                `link`)])()
# 
...
CountryCities <-
  fread("Raw-data/worldcities.csv", na.strings = "", sep=",",
        select = c("country","city_ascii","capital")
  )[capital != "primary",
    capital := NA_character_
  ][!is.na(capital)
  ][, `Capital City` := unique(city_ascii),
    by = "country"][]
...
As we could see some countries have some countries refer to the region column with a different name. That produces a lot missing value in the region column after binding the results despite the data frame has many of those values with a different name as we can see bellow.

```{r missing-region check}
...
To solve this problem we just need to save in a vector the columns related to regions with a different name and check if a value they have it's missing in the region column and if that is the case we copy that value to the region column.

```{r filling-region-column}

# OtherRegionColumn <- 
#   setdiff(names(CountryCities),
#           c("City","Region","Population","link"))
# 
# for (var_name in OtherRegionColumn) {
#   
...
After applying those changes we can see that all missing Region columns doesn't have any other from it could the data from, we can delete does columns and join the results with the `CountryCapital` data.

```{r joining-capital-cities}

CountryCapitalCities <-
  CountryCities[, (OtherRegionColumn) := NULL
  ][, `:=`(City = str_remove(City," / Capital$"),
           Population = clean_amount(Population))
  ][CountryCapital, on = "link"]
...
# fwrite(CountryCapitalCities,"Raw-data/Country-Capital-Cities.csv")
...
CountryCapitalCities <-
  fread("Raw-data/Country-Capital-Cities.csv", na.strings = "")
...
CountryCapitalCities[, .(city_regex = City |> 
                                      stringi::stri_trans_general("Latin-ASCII") |>
                                      (\(x) fcase(x == "Santiago de los Caballeros",
                                                  "Santiago",
                                                  x == "San Juan de la Maguana",
                                                  "San Juan",
                                                  rep(TRUE,length(x)),
                                                  x))() |>
                                      str_c(collapse = "|")),
                     by = c("Country","Capital City")
][ProtestReshaped, on = c("Country"="country")
][, `:=`(in_capital = location %like% `Capital City`,
         cities_found = str_match_all(location,city_regex) |> unlist() |> length(),
         location_no_found = str_remove_all(location,city_regex)),
  by = id
][,location_no_found :=  str_remove_all(location_no_found,
                                        str_c(c("[:punct:]",
                                                " and($| )",
                                                str_c(" ",tm::stopwords("en"),"($| )")), 
                                              collapse = "|")) |> 
                         str_squish(),
][! location_no_found %like% "\\w",
  location_no_found := NA_character_
][!is.na(location_no_found) & Country == "China", unique(location)]
  .(id,Country,location,location_no_found,cities_found,in_capital)]
