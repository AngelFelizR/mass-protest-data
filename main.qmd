...
## What correlation que can find between protestan demands and state respose?


```{r}

ProtestLocationExtracted[, !c("id","ccode","democ","autoc")
][, lapply(.SD, \(x) x * 1),
  .SDcols = \(x) is.logical(x) | is.numeric(x)
][, correlation::correlation(.SD) |> as.data.table()
][p < 0.05
][order(-abs(r)),
    .(relation = fct_reorder(paste0(Parameter2," <-> ",Parameter1), abs(r)),
      r = abs(r), 
      CI_low = abs(CI_low), 
      CI_high = abs(CI_high),
      direction = fifelse(r >= 0, "Positive", "Negative") |>
                  factor(levels = c("Positive", "Negative")))
  ][, rbind(.SD[direction == "Positive"][1:10],
            .SD[direction == "Negative"][1:10])] |>
  ggplot(aes(r, relation, color = direction))+
  ...
  expand_limits(x = 0)+
  scale_color_manual(values = c("Positive" = "dodgerblue3",
                                "Negative" = "brown2"))+
  labs(title = "Most important correlations",
       y = "", x = "Correlation", color = "Direction")+
  theme(legend.position = "top")

```

As we can see the state try to dispersal the crowd
