...
As we know in the column `location` there is information about the place where each protests took place. The information is store in plain text so we will need to use regular expression to extract some information about it. with this code we will be able to know the number of sites where the protest took place, if the protest took place in the capital and if the protest was a national movement or just happened in specific cities.
...
ProtestCitiesCount <-
  ...
  ][, `:=`(in_capital = location %like% capital_city,
           is_national = location %like% str_c(NationalRegex, country),
           num_cities = str_match_all(location, city_regex) |> unlist() |> uniqueN(),
           location_no_found = str_remove_all(location, city_regex) |>
                               str_remove_all(str_c(NationalRegex, country)) |>
                               str_squish()),
    by = "id"
  ][!location_no_found %like% "\\w" |
      location_no_found %chin% c(stop_words$word, tm::stopwords("spanish")),
    location_no_found := NA_character_]

```

## Extracting protesteridentity data

```{r}

AproximationCities <-
  merge(ProtestCitiesCount[!is.na(location_no_found),
                           .(id, country, location, location_no_found)],
        WorldCities[,.(country, city_ascii)],
        by = "country", allow.cartesian = TRUE
  )[, weight := 
      1 - 
      stringdist(location_no_found,city_ascii, method = "lcs") /
      (nchar(location_no_found)+nchar(city_ascii))
  ][weight >= 0.8
  ][order(-weight),
    unique(.SD, by = "id")]
